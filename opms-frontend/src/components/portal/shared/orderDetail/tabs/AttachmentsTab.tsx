...
type AttachmentsTabProps = {
  ...
  /** When `dispatch_filtered`, only account/finance/dispatch attachments are shown. */
...
const DEPARTMENT_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  ...
  account: { bg: "bg-indigo-50 dark:bg-indigo-950/20", text: "text-indigo-700 dark:text-indigo-400", border: "border-indigo-100 dark:border-indigo-950/30" },
...
const DISPATCH_VISIBLE = new Set(["account", "finance", "dispatch"]);
...
export default function AttachmentsTab({
  ...
  const visibleAttachments = useMemo(() => {
    ...
    return attachments.filter((att) => {
      ...
      return Boolean(dept && DISPATCH_VISIBLE.has(dept));
    });
  ...
  const grouped = useMemo(() => {
    ...
    for (const att of visibleAttachments) {
      ...
      if (groups[key]) groups[key].push(att);
    }

    return Object.entries(groups).filter(([_, list]) => list.length > 0);
  }, [visibleAttachments, visibility]);
