...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (companyName) {
          document.title = `${companyName} - Lead Manager Sign In`;
        }
      } catch {
        /* keep defaults */
      }
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (!hasLeadManagerPortalAccess(res.user)) {
        throw new Error(
          "Access denied: You do not have permission for the Lead Manager portal.",
        );
      }
  ...
  const companyTitle =
    companyInfo?.trade_name || companyInfo?.legal_name || "Lead Manager Portal";
    ...
    <div className="flex min-h-screen flex-col justify-center py-12 sm:px-6 lg:px-8 bg-background font-sans text-foreground">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center space-y-3">
        ...
        <p className="text-xs text-muted">
          Lead Pipeline & Quotations Management
        </p>
      </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-8 shadow-2xl backdrop-blur-xl">
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="rounded-xl border border-primary/20 bg-primary/10 p-3 text-[11px] text-primary flex items-center gap-2">
              ...
              <span>Sign in with your Lead Manager portal credentials.</span>
            </div>
