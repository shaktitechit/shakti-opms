...
import { Calendar, Lock, Mail, ShieldCheck, ArrowRight } from "lucide-react";
...
import type { ThemeColor } from "@/types/workPlanner";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (companyName) {
          document.title = `${companyName} - Work Planner Sign In`;
        }
      } catch {
        /* keep defaults */
      }
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (!hasWorkPlannerPortalAccess(res.user)) {
        throw new Error(
          "Access denied: You do not have permission for the Work Planner portal.",
        );
      }
  ...
  const companyTitle =
    companyInfo?.trade_name || companyInfo?.legal_name || "Work Planner Portal";
    ...
    <div className="flex min-h-screen flex-col justify-center py-12 sm:px-6 lg:px-8 bg-background font-sans text-foreground">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center space-y-3">
        ...
        <p className="text-xs text-muted">
          Field Visit & Expense Management
        </p>
      </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-8 shadow-2xl backdrop-blur-xl">
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="rounded-xl border border-primary/20 bg-primary/10 p-3 text-[11px] text-primary flex items-center gap-2">
              ...
              <span>
                Sign in with your Work Planner manager or executive portal
                credentials.
              </span>
            </div>
